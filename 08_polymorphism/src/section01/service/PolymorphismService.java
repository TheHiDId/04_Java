package section01.service;

import section01.dto.Child;
import section01.dto.Parent;

public class PolymorphismService {
	/*
	 * [매개 변수의 다형성]
	 * 매개 변수의 타입을 부모 타입으로 설정하여 
	 * 모든 자식 객체 참조 주소를 전달받을 수 있도록 함 (업캐스팅)
	 * 
	 * 매개 변수(Parameter): 메서드 호출 시 전달된 값을 저장하는 변수
	 */
	
	/*
	 * 참조변수 instanceof 클래스명
	 * 참조변수가 참조하는 객체가 클래스로 만들어진 객체가 맞을 경우 참 아니면 거짓
	 * 상속 관계 파악 가능 -> 자식 객체 내부의 부모 객체도 인식
	 */
	
	/**
	 * 객체의 타입, 필드 정보를 출력하는 메서드
	 * @param obj: Object 또는 Object를 상속받은 객체 참조 변수
	 */
	public void printObject(Object obj) {
		String result = null;
		
		if(obj instanceof Child) {
			Child c = (Child)obj;
			
			result = "[Child Type]\n";
			result += c.getLastName() + " / " + c.getMoney() + " / " + c.getCar();
		} else if(obj instanceof Parent) {
			Parent p = (Parent)obj;
			
			result = "[Parent Type]\n";
			result += p.getLastName() + " / " + p.getMoney();
		}	else {
			result = "[Object Type] 필드 없음";
		}
		
		System.out.println(result);
	}
	
	public void test1() {
		Object obj = new Object();
		Parent parent = new Parent("김", 500);
		Child child = new Child("이", 6000, "소나타");
		
		printObject(obj);
		printObject(parent);
		printObject(child);
	}
	
	/*
	 * [반환형의 다형성]
	 * 메서드의 반환형을 부모 타입으로 설정하여 반환되는 자식 객체의 주소를 참조할 수 있게함
	 */
	
	/**
	 * 전달 받은 num 값에 따라 알맞은 객체를 생성하고 반환하는 메서드
	 * @param num: 0: Child, 1: Parent, 나머지: Object
	 * @return 생성된 객체 참조 주소 반환
	 */
	public Object getInstance(int num) {
		if(num == 1) return new Child();
		
		if (num == 2) return new Parent();
		
		return new Object();
	}
	
	public void test2() {
		Object o1 = getInstance(1);
		Object o2 = getInstance(2);
		Object o3 = getInstance(3);
		
		printObject(o1);
		printObject(o2);
		printObject(o3);
	}
	
	/*
	 * [바인딩(Binding)]
	 * Binding: 연결하다, 묶다
	 * 메서드 호출부와 실제 실행할 메서드 코드를 연결하는 것
	 * 
	 * [정적 바인딩]
	 * 프로그램 실행 전, 컴파일 단계에서 메서드와 메서드 호출부를 연결
	 * 
	 * [동적 바인딩]
	 * 프로그램 실행 중(Runtime), 메서드 호출부를 실제 참조하는 객체에 오버라이딩된 메서드와 연결
	 * 
	 * 사용 이유
	 * 1. 다운캐스팅의 번거로움을 없앰
	 * 2. 재정의된 메서드(더 효율적 또는 알맞은 기능)를 수행
	 * 
	 * 상속, 오버라이딩, 업캐스팅(+ 메모리 구조)
	 */
	public void test3() {
		Parent p1 = new Parent("김", 10);
		Child c1 = new Child("최", 20, "캐스퍼");
		Parent p2 = new Child("박", 30, "포터"); // 업캐스팅
		
		System.out.println("p1.toString(): " + p1.toString());
		System.out.println("c1.toString(): " + c1.toString());
		System.out.println("p2.toString(): " + p2.toString()); // 정적 동적 상태에 바인딩이 변경됨
	}
	
	/*
	 * [객체 배열의 다형성]
	 * 부모 타입의 참조 변수를 묶음으로 다루어서 배열 각 요소에 업캐스팅 적용
	 * 
	 * 객체 배열: 객체 참조 변수의 묶음
	 */
	public void test4() {
		// Object 객체 참조 변수 5칸 짜리 배열 생성해서 Object[] 참조 변수 arr에 배열 주소 대입
		Object[] arr = new Object[5];
		
		for(int i = 0; i < arr.length; i++) {
			arr[i] = getInstance(i % 3 + 1); // 0 1 2 3 4 / 1 2 3 1 2
		}
		
		System.out.println();
		
		for(Object obj : arr) {
			System.out.println(obj.toString()); // 동적 바인딩
		}
		
		System.out.println();
		
		// 동적 바인딩 기능이 없다면?
		for(Object obj : arr) {
			if(obj instanceof Child) {
				System.out.println(((Child)obj).toString());
			} else if(obj instanceof Parent) {
				System.out.println(((Parent)obj).toString());
			} else {
				System.out.println(obj.toString());
			}
		}
	}
	
	/*
	 * 다운 캐스팅 시 주의 사항
	 * 강제 형변환이 적용되는 참조 변수가 형변환 하려는 타입의 객체를 참조하고 있는지 확인이 필요
	 * instanceof 사용
	 */
	public void test5() {
		Parent p = new Child("김", 200, "소나타");
		
		test6(p);
	}
	
	// 전달받은 객체를 String 타입으로 다운캐스팅
	public void test6(Object obj) {
		if(obj instanceof String) {
			String p = (String)obj;
			
			System.out.println(p);
		} else {
			System.out.println(obj.getClass().getName());
		}
	}
	
	/*
	 * debug 모드
	 * 이클립스 등 IDE에서 제공하는 기능
	 * 코드 수행 중, 지정된 지점(BreakPoint)에서 실행을 멈추고 
	 * 해당 시점에 존재하는 변수 또는 필드 값을 확인하는 모드
	 */
}
